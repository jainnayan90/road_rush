class SoundButtons extends Phaser.GameObjects.Container {
	// if container is not allowed inside container in future remove extends and super
	constructor(config){
		super(config.scene);
		this.scene = config.scene;

		this.musicButton = new ToggleButton({scene:this.scene, backKey:'toggleBack', onIcon:'musicOn', 
                                            offIcon:'musicOff', event:GC.TOGGLE_MUSIC});
		this.sfxButton = new ToggleButton({scene:this.scene, backKey:'toggleBack', onIcon:'sfxOn', 
                                            offIcon:'sfxOff', event:GC.TOGGLE_SOUND});
